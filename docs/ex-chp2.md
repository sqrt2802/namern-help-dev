...
假如你选择了跳过 Chapter 1，那么请严格遵守我对这些工具使用时的推荐数值以及用法，即使你不明白为什么。如果你没跳过 Chapter 1，但是仍然不知道为什么，那也请遵守我对这些工具使用时的推荐数值。或许你在看到 Chapter 3 的时候会明白的。
...
### 2.1.1 你为什么应该选择 pbb 测号器？

（可能并不）众所周知，`魂魄妖梦 AKG1OENX@Squall` 是八围不到 650 的“无表号”，但是它的强评高达 6595，一度在榜单上排名强评第一好几个月。而此号的测出，得益于由曙光和 pl 最初撰写，由我和曙光进行进一步改良的 pbb 测号器。

在使用 pl 测号器的过程中，由于降低八围阈值意味着需要多测更多占比的虚评，进而会导致速度下降，很少有人会将八围阈值降得如此低，那么，是否就有可能错过这样的超级强号？  
...
**请尽量选择最新的测号器，截止写这段教程时（20250420）时最新的测号器为 pbb5.2.1**  
...
DEBUG 模式是在有人反馈测号器出 bug 的时候我会选择让他开启然后告诉我输出信息的东西，建议别开，只选择 0 或者 2。区别在于能否动态改变线程数。  
...
为了方便一些人（其实包括我）对同时测多个前缀后缀的需求，我开发了多前缀后缀模式。  
...
这段话有点过时，但是我懒得改了。我的意思是，我推荐只使用 utf-8 编码输出。因为很多后续的工具是需要直接读入 utf-8 编码的。
...
对于长度为 1 2 3 4 的字符集，我预设了一些你可能会需要用到的字符集，当然如果没有的话你也可以自行输入。
...
如果你需要的字符集并不在我预设的范围内，你可以输入 7 或者 8。**注意这两个不要同时输入，且如果和其他的一起输入的话，放在最后！**  

如果你输入 7 那就按指示做，我不再赘述了。如果你输入 8。
```
...
我搞这种东西是为了方便我之前所说的多前缀后缀的测号格式。正常来说你需要对每一个前后缀格式输入一个区间，但是考虑这个区间有可能有大量重复，所以我增加了可以一次性输入多个区间。
...
我的推荐阈值：xp 4900~5000 xd 5600~6000。

值得注意的是对于某些特殊类型的号（指分身护符魅惑苏生），我会将 xp 阈值降低 300（这个东西写在代码内部了）。至于原因，你将会在第三节看到。  
...
不建议在输出文件中显示虚评虚单，因为很多后续工具需要纯粹的号座位输入。  
忽略“建议不懂的人选 0”这句话，我只是忘记删了而已。后续很多工具需要 utf-8 编码的原号文件。  
后两个东西显示在屏幕上应该更好一些。  
...
输出会以 append 的形式追加到新的文件中，不需要担心被覆盖。  
...
自己查看自己电脑的 cpu 型号，看看有几个核，几个线程可以用。名竞群其他群友一般推荐总线程数-2 之类的，但是我认为也可以直接输入总线程数。如果不想开满也可以输入更低的数。  

随后会输出几个号让你看一下格式大概是怎样的，即，看看你的格式有没有搞错。但是如果你的字符集包含非 ANSI 的字符可能会显示莫名其妙的东西。

然后如果你按我说的选择显示在屏幕上的话，大概会跳出来类似这样的东西。
```
...
我在这里简要说一下 pbb 多线程的原理。他创建了多个线程。将测号任务以 1e6 为单位分割成任务，然后每个线程会接收任务，当自己这边的任务执行完了之后会继续接收新的任务。所以这个 task90 finished 就是第几个任务被完成了。注意到完成任务的顺序并不一定完全就是布置任务的顺序。  
...
严格来说核心技术是一个提供了接口的 js 文件（`md5.js`），如果有技术力的话可以自行想办法改成 bun 运行。  
示例用法放在 `example.js` 里了，想要自行开发的话视情况自行调用。  
**注意：这个东西运行后有内存泄漏的风险，想要自行开发的一定要留意这一点！！！**
...
需要注意的是，由于 cpp 生成的可执行文件本身无法控制 cmd 创建的 `node.exe` 的多线程调度（尤指输出的原子性），我将输出按照文件放在了 `out` 文件夹下，每次子线程只**以追加形式**输出在自己对应的那个文件里。最后你需要双击运行 `merge_files.bat`，然后输入你想要汇总到哪个文件（如 `output.txt`），进行汇总。  
这也意味着你如果不清理 `out` 文件夹，会导致几次输入的结果混在一起。我在几个可执行文件的开头都放了清空 `out` 文件夹的选项，既是方便地进行清空，也有提醒的作用。当然如果你认为不应该清空可以选择不清空。  
同时由于我进行多线程的原理是将大文件拆成多个小文件存在 `tmp` 文件夹里再让 `node.exe` 去读取，这会导致你**无法**在一个 `node-openbox` 文件夹内同时运行多个 exe，会出较为严重的 bug。
...
准确度表示进行前多少**百**场战斗然后算平均值，比如输入 100 表示一万场战斗，精确到四位有效数字，这个和测官方评分时的 100% 是一致的。  
...
```
类型为 `all` 的时候阈值为强评的阈值。  
考虑到有些人是急急国王（比如我），很想在整个测号任务完整测完前就得知一部分强号的信息，我提供了输出在屏幕上的阈值这一个东西。  
我的建议是，你的这个阈值应当 >= 输出在文件中的阈值，毕竟我设计出来只是为了方便你偷看一部分强号的数值的。
...
你需要输入你测的总号量，以及**输入文件**的文件名，包含后缀（如`.txt`），输出文件名并不会再这里输入，而是在 `merge_files.bat` 里输入！
输入文件内的输入格式为每个号结尾换行。    
Q: 为什么需要输入总号量，但是 XP 和 convert 之类的就不需要？
A: 为了实现方便，解释起来比较复杂，不解释了。归根到底就是我实现比较狂野且懒得改进。
...
在 cqp.js 中，在 `if (team_size == X)`（X 为 1 或 2）后，会有 `target = [...]` 的语句，如果想要修改靶子，你只需要改变里面的内容。每个号/组合开头和结尾加引号，然后加逗号。**注意有些字符在代码里不应该直接输入，需要将一些字符的前面加上\\，如'"\\`等。** 不然会导致号不正确甚至编译错误。

**关于 diy cqp**

有些情况下你可能想要测使用了 diy 的二人组，那你可以选择 `diy-cqp.exe` 输入的时候两个号都用 diy 格式，中间用 `++` 而不是单个 `+` 连接。  
...
Data\_Structure 是在 刺评/辅评/地评 等基于 cqp 的评分模型的基础上开发的二人组一键配队整合包。  
...
但是测 cqp 是慢的。借鉴 xp，我训练了简单的拟合模型来根据号的 43 维属性预测上文所提及的若干评分，均被称为 `s?p`，丝某评。
...
也在 Tools 文件夹。和 1.7 的区别在于，这里介绍的工具大多是你阅读了第二章之后才理解它的意义和用法。

### 2.4.1 SP\_all

之前提到过的 SP，可以粗略地预测一个号作为某个类型的号的时候的评分。  
输入在 `input.txt` 里，每一行一个号然后换行。  
点开 `exe` 之后会有提示，让你输入一个数字，分别表示某一个种类的丝评，一共 7 种。同时输入 0 表示对超强单的预测。  
...
这个就是含有潜力值的 SP，之前提到过了。没有“偷看器”和超强单的预测。  
输入在 `input.txt` 里，每一行一个号然后换行。  
输出在 `SP_ptt.txt` 里。格式为 `原分数 潜力分数 号`。  
