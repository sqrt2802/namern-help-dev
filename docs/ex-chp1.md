...
举例，假如一个号只有两个主动技能，技能 A 和技能 B。技能 A 和 技能 B 的熟练度分别为 64 和 96。在两个技能发动条件都满足的情况下，如果 A 在前，B 在后，那么发动 A 的概率为 $1/2$，发动 B 的概率为 $(1-1/2)\times 3/ 4 = 3/8$。 如果 B 在前，A 在后，那么发动 B 的概率为 $3/4$，发动 A 的概率为 $(1-3/4)\times 1/2= 1/8$。
...
特别地，最后一个主动技能的熟练度翻倍（如果存在熟练度不为 0 的主动技能）。  
假如最后一个主动技能不是最后一个技能，那么会将熟练度加某两个新的 name_base 的最小值，但是不超过原技能本身的熟练度。  
假如最后一个主动技能不是倒数第二个技能，那么会将熟练度加某两个新的 name_base 的最小值，但是不超过原技能本身的熟练度。  
倒数第三再往前没有类似的操作。  
...
至于“最好”这个概念怎么衡量，我们采用模型评分，比如虚评，或者一些其他的我开发的评分。具体你会在 2.3 中见到。
...
根据玩家【新纪元】研究发现，一个号的 (攻-防)+(魔-抗)+速+(敏+智)\*0.5，这项数值对对手的索敌十分关键。我们称其的两倍为**“嘲讽值”**。（群里有部分人不乘二，我习惯乘二，这样就一定是整数）

我们只讨论二人组，在二人组中如果一个号的嘲讽值高于另一个号，那么在对战中会**显著**更容易被**大部分攻击**索敌（有些技能的索敌机制不一样）。并且这样的嘲讽值差距是**突变**型的而非一个平滑的曲线。也就意味着，仅仅 2 的属性改变可能会造成胜率，或者说强度的巨大差异。
...
将一堆号丢进 xp 或者 convert 的 html 里的时候会测得很慢，所以我开发了 cpp/exe 版本。
...
同时我提供了 cpp 源代码，可以自己进行修改，然后重新编译。
...
注意我没有进行是否是同队的判断，请你自行保证输入的多人组是同队的。
...
输入在 `input.txt` 里，每一行一个号然后换行。  
输出在 `base.txt` 里。

### 1.7.4 sort

后面这些东西其实和名竞无关了，但是有些人确实是不会编程的，因此我也写了。
...
双击打开 UltraCodingSwitch.exe。这个工具和我自己编写的各种草台工具不一样，有明确的 UI，可以自己看。