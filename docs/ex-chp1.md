...
<details><summary>点击展开解答</summary>

对于另一个号，如果对应 y 这个位置的 name_base 值为 x，那么自己和对方都会出现一次取 max 操作，更容易出现双加成。  
在特定情况下还会容易出现，原来两个属性相同，加成后两个属性还是相同。  

举例：一个号对应 攻 的三个 name_base 是 `40 63 40`，另一个号则是 `20 40 61`，那么这两个号加成后的 name_base 是 `40 63 61` 和 `20 63 61`，最终属性都从 76 变成了 97。
