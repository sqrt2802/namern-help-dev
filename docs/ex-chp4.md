...
最开始我们将 two 改成了枚举二人组后计算虚评之和，然后取高的（一般是大于 12300）进行开超强评。但是一些强评和虚评不那么高但是实战很强的号让这个技术始终有一定的局限性。  

在这种情况下我们重新沿用了 XX化 的思路，通过将某一类型的号和对应适配类型的一个基准号配合起来测 1% cqp 作为衡量强度的标准，但是这一次我们选择根据这个测试出来的数据集训练出了几个模型。  

在评判一个二人组实力的时候，用的也不是虚评和，而是新模型的分数之和，这样得出来的结果更加合理。随后对于不同的种类设置不同的阈值即可。

值得注意的是这个时候这样的配队并不能包含人海组，因为人海组非常怪，而且模型训练很不准。

#### 4.3.6.2 Data\_Structure

已经介绍过了。实际上就是将 Two 的过程给自动化，同时增加了“增量枚举二人组”，使得它操作更简单并且减少了很多重复工作。

与此同时，添加了对人海组的支持。具体来说就是，虽然分身号的评分很不准，但是也并没有那么不准，至少我们可以先用它作为初筛选出一批有可能强的号，随后把它们和对应配件号拿去配，测 1% 的 cqp。最终在运行 two 的时候，对于一个加成过的号，我们认为他的分数是 （最终模型分数-初始模型分数）+初始实际评测分数。这就是为什么在人海组里需要 openbox 进行开箱。

### 4.3.7 双人的基本克制关系


写在前面：名竞一个号有属性+技能共四十余个参数，双人的复杂程度比单人更甚，漏掉一些特例性情况已是在所难免。所以下面仅仅是**尝试**从战斗角度分析各大主流技能的特点。
